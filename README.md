...
针对 Codex / ChatGPT 导出的原始会话 ZIP，提供不可变归档到 Obsidian Markdown 笔记、派生 SQLite FTS / 向量索引、以及 Agent 混合召回的完整管道：

```text
Immutable ZIP -> CodexExportReader -> Obsidian Markdown -> Heading-aware Chunker -> SQLite FTS/Vector -> MCP
```
...
```powershell
# 1. 审计导出 ZIP 完整性与会话统计
python scripts/conversation_cli.py audit-export "path/to/export.zip"

# 2. 模拟导入（Dry-run）
python scripts/conversation_cli.py import "path/to/export.zip" --dry-run

# 3. 扫描附件引用并生成缺失报告（JSON / CSV）
python scripts/conversation_cli.py inventory-attachments "path/to/export.zip" --missing-only --output-json attachments.json

# 4. 从已生成的 Markdown 全量重建派生索引
python scripts/conversation_cli.py rebuild-index --confirm-rebuild

# 5. 执行混合检索
python scripts/conversation_cli.py search "324 nm"

# 6. 验证生成的 Markdown 格式
python scripts/conversation_cli.py validate

# 7. NAS bge-m3 Embedding 连通性测试 (显式、可跳过)
...
`conversation_archive.manifest_path` config value is retained only for backward
configuration compatibility and is not used by production import/index flows.

Manual note content after the autogenerated block is part of the conversation
search index. Editing manual notes does not create an import conflict, but it
does make the derived index stale so the new note text becomes searchable after
the next incremental index run.

### 扩展 MCP 工具（可选，启用 conversation_archive.enabled 时生效）

- `conversation_search`：基于关键词 FTS5 与向量余弦相似度混合搜索会话切片。
- `conversation_read`：读取指定会话 Note 正文或标题小节（严格目录边界保护）。
- `conversation_recall`：在指定 token budget 内提取格式化会话上下文，带 source anchor 溯源标记。

#### MCP 工具调用说明

| 工具名称 | 参数 | 说明 |
|---|---|---|
| `conversation_search` | `query` (必需), `project`, `conversation_id`, `parent_thread_id`, `limit` | 词法与向量双路召回，自动根据分数融合排序并附加 source anchors。 |
| `conversation_read` | `file_path` (必需), `heading` (可选) | 受到 `allowed_roots` 目录安全沙箱保护，仅允许读取已授权的 Markdown 笔记。 |
| `conversation_recall` | `query` (必需), `token_budget` (默认 1500), `project`, `conversation_id`, `parent_thread_id` | 严格遵守 token budget，格式化输出带有 session ID、heading 与 anchor 标记的上下文文本。 |
...
conversation_archive:
  enabled: true
  # Obsidian Vault 绝对路径（容器环境需映射并使用 Linux 路径）
...
webui:
  enabled: false  # 默认可选启用；内网受信任环境可开启
  host: 0.0.0.0
  port: 8788
```
...
WebUI 现提供专用的 Conversation Memory 只读控制台：

1. **状态卡片**：实时展示归档会话文档总数、内容分块数、向量数、覆盖率及活跃嵌入模型规格。若 embedding 未启用，清晰提示 `Lexical only`，不产生错误告警。
2. **会话搜索**：输入实验关键词、附件文件名、错误码或 UUID，展示匹配切片、相关性分数类型（Hybrid / Lexical / Vector）、以及明确的定位锚点溯源标记（Anchored provenance）。
3. **召回预览 (Recall Preview)**：直接复用 MCP `conversation_recall` 逻辑，在用户调整 Token 预算时，实时预览实际注入给 Agent 的上下文文本与字符预算占用。
4. **笔记阅读器 (Conversation Reader)**：点击搜索结果一键查看 Markdown 完整文档或当前命中标题小节，受沙箱路径限制，安全隔离不可读路径。

> **生产安全提醒**：WebUI 严格遵循**只读原则**，不提供网页端全量导入 (Bulk Import) 或全量向量化 (Bulk Embedding) 按钮，避免长时间网络阻塞或意外触发重跑。批量导入继续由 CLI 命令与显式确认参数门控负责。

### NAS 部署与数据挂载指南

在将服务部署至 NAS (如 Unraid) 容器环境时，**镜像本身不包含本地对话归档数据**。必须确保：

1. **路径转换**：Windows 下的本地路径（如 `D:\Partition\...\Vault`）**不可直接写入 NAS 容器配置**，必须使用 Linux 挂载点路径。
2. **数据卷映射**：将 NAS 上的 Markdown 笔记目录只读挂载给容器，并将 SQLite 索引持久化保存在 NAS data 卷中。
...
services:
  research-memory-gateway:
    image: ghcr.io/kettly1260/research-memory-gateway:v0.2.0
    ...
    volumes:
      ...
      - ./data:/app/data
      - /mnt/user/appdata/research-memory-gateway/conversations:/conversation-vault:ro
    environment:
      - TZ=Asia/Shanghai
```
